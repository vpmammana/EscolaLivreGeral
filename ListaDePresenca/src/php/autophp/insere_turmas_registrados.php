...
<html>
<head>
<title>Inserção de dados na tabela (maio-2020): turmas_registrados ".$campo_busca."</title>
...
<body id='conteudo'>
<div class='cabecalio'>
<h1>Inserção de Dados na tabela (maio/2020): turmas_registrados</h1>
...
$sql_comment="SELECT TABLE_COMMENT FROM information_schema.TABLES WHERE TABLE_NAME = 'turmas_registrados';";
...
$sql="select count(*) from turmas_registrados";
...
<div style='border: 1px solid black; background-color: green; width: 100%'><table style='width: 100%'><tr><td>Consulta:<input type='text' id='id_campo_de_search' data-nivel='0' placeholder='Coloque a consulta aqui!' /></td><td><input 
				type='button'
				value='id_chave_turma_registrado' 
				data-nivel='0' 
				name='busca_principal'
				onclick='var itz=document.getElementById(`id_campo_de_search`).value;  if (itz===``) {alert(`Você deixou o campo de busca vazio!`);} else {carrega_busca(`id_chave_turma_registrado`,itz,50,0);;}'
			></td><td> <input 
				type='button'
				value='nome_turma_registrado' 
				data-nivel='0' 
				name='busca_principal'
				onclick='var itz=document.getElementById(`id_campo_de_search`).value;  if (itz===``) {alert(`Você deixou o campo de busca vazio!`);} else {carrega_busca(`nome_turma_registrado`,itz,50,0);;}'
...
<tr class='cabecalio_table'><th></th><th>id</th><th>nome_turma_registrado</th><th>id_turma</th><th>id_registrado</th><th>id_tipo_vinculo</th></tr>";
...
if ($campo_busca=="") {$sql="select * from turmas_registrados order by nome_turma_registrado limit ".$limit." offset ".$offset;}
else {$sql="select * from turmas_registrados where ".$campo_busca." like '".$valor_busca."%' order by '".$campo_busca."' limit ".$limit;}
echo $sql;
$result=$conn->query("$sql");
...
$id_chave_turma_registrado=$row["id_chave_turma_registrado"];

$id_registro=$row["id_chave_turma_registrado"];

$nome_turma_registrado=$row["nome_turma_registrado"];
$campos_atualizaveis=$campos_atualizaveis.",".'"campo_'.$id_registro.'_2"';
$id_turma=$row["id_turma"];

$id_registrado=$row["id_registrado"];

$id_tipo_vinculo=$row["id_tipo_vinculo"];

$campos_atualizaveis=$campos_atualizaveis."]";
echo "<tr class='principal'><td>".$row_number."</td><td>".$id_chave_turma_registrado."</td><td>
				<input 
					...
					value='".$nome_turma_registrado."'
					data-alterado='nao'
					data-tabela='turmas_registrados'
					data-campo='nome_turma_registrado'
                                        data-id='".$id_registro."'
        				data-nivel='0'
				/>
			   </td>
...
<div class='dropdown'>
  <input type='text' 
	...
	data-fkid='".$id_turma."' 
        data-default=''
	data-fk-banco='escolax' 
	data-fk-tabela='turmas' 
	data-fk-id='id_chave_turma'
	data-selecionado='-1'
	data-event-blur='NAO'
...
<div class='dropdown'>
  <input type='text' 
	...
	data-banco='escolax' 
	data-tabela='turmas_registrados'
	data-campo='id_tipo_vinculo' 
	data-fkid='".$id_tipo_vinculo."' 
        data-default=''
	data-fk-banco='escolax' 
	data-fk-tabela='tipos_vinculos' 
	data-fk-id='id_chave_tipo_vinculo'
	data-selecionado='-1'
	data-event-blur='NAO'
	data-event-focus='NAO'
	data-event-keyup='NAO'
...
mostra_botao('insercao','turmas_registrados','0');
...
for (i=0; i<pontos.length; i++){
	...
	carrega_NtoN(pontos[i].id,'escolax', 'turmas_registrados','id_documento',document.getElementById(pontos[i].id).innerText);
...
ativa_alterados();
if ('turmas_registrados'=='documentos'){percorre_ponto_insercao();}
...
mensagem_de_carregamento_ativada();
  ...
  alert('Busca pelo campo '+campo_busca);
	var resposta='';
	var url='insere_turmas_registrados.php?offset='+offset+'&limit='+limit+'&campo_busca='+campo_busca+'&valor_busca='+valor_busca+'&scrollx='+escrolx+'&scrolly='+escroly;
        var oReq=new XMLHttpRequest();
	oReq.open('GET',url, false);
	oReq.onload= function (e) {
			    resposta=oReq.responseText;
...
function (){
	var resposta='';
	var url='insere_turmas_registrados.php?offset=0&limit=".$limitador_registros_insere."&campo_busca=".$campo_busca."&valor_busca=".$valor_busca."&scrollx='+escrolx+'&scrolly='+escroly;
        var oReq=new XMLHttpRequest();
	oReq.open('GET',url, false);
	oReq.onload= function (e) {
			    resposta=oReq.responseText;
...
var resposta='';
var url='apaga_registro.php?banco=escolax&tabela=turmas_registrados&id='+id;
alert(url);
var oReq=new XMLHttpRequest();
oReq.open('GET', url, false);
oReq.onload= function (e) {
