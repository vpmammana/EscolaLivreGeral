...
<html>
<head>
<title>Inserção de dados na tabela (maio-2020): presencas ".$campo_busca."</title>
...
<body id='conteudo'>
<div class='cabecalio'>
<h1>Inserção de Dados na tabela (maio/2020): presencas</h1>
...
$sql_comment="SELECT TABLE_COMMENT FROM information_schema.TABLES WHERE TABLE_NAME = 'presencas';";
...
$sql="select count(*) from presencas";
...
<div style='border: 1px solid black; background-color: green; width: 100%'><table style='width: 100%'><tr><td>Consulta:<input type='text' id='id_campo_de_search' data-nivel='0' placeholder='Coloque a consulta aqui!' /></td><td><input 
				type='button'
				value='id_chave_presenca' 
				data-nivel='0' 
				name='busca_principal'
				onclick='var itz=document.getElementById(`id_campo_de_search`).value;  if (itz===``) {alert(`Você deixou o campo de busca vazio!`);} else {carrega_busca(`id_chave_presenca`,itz,50,0);;}'
			></td><td> <input 
				type='button'
				value='nome_presenca' 
				data-nivel='0' 
				name='busca_principal'
				onclick='var itz=document.getElementById(`id_campo_de_search`).value;  if (itz===``) {alert(`Você deixou o campo de busca vazio!`);} else {carrega_busca(`nome_presenca`,itz,50,0);;}'
			></td><td> <input 
				type='button'
				value='data_tempo' 
				data-nivel='0' 
				name='busca_principal'
				onclick='var itz=document.getElementById(`id_campo_de_search`).value;  if (itz===``) {alert(`Você deixou o campo de busca vazio!`);} else {carrega_busca(`data_tempo`,itz,50,0);;}'
			></td><td> <input type='button' data-nivel='0' name='busca_principal' value='ultimo'/></td><td> <input type='button' value='Todos' data-nivel='0'/></td></tr></table></div>
";
...
<tr class='cabecalio_table'><th></th><th>id</th><th>nome_presenca</th><th>id_registrado</th><th>id_evento</th><th>data_tempo</th><th>time_stamp</th></tr>";
...
if ($campo_busca=="") {$sql="select * from presencas order by nome_presenca limit ".$limit." offset ".$offset;}
else {$sql="select * from presencas where ".$campo_busca." like '".$valor_busca."%' order by '".$campo_busca."' limit ".$limit;}
echo $sql;
$result=$conn->query("$sql");
...
echo "<tr class='principal'><td>".$row_number."</td><td>".$id_chave_presenca."</td><td>
				<input 
					...
					value='".$nome_presenca."'
					data-alterado='nao'
					data-tabela='presencas'
					data-campo='nome_presenca'
                                        data-id='".$id_registro."'
        				data-nivel='0'
				/>
			   </td>
...
<div class='dropdown'>
  <input type='text' 
	...
	data-banco='escolax' 
	data-tabela='presencas'
	data-campo='id_evento' 
	data-fkid='".$id_evento."' 
        data-default=''
	data-fk-banco='escolax' 
	data-fk-tabela='eventos' 
	data-fk-id='id_chave_evento'
	data-selecionado='-1'
	data-event-blur='NAO'
	data-event-focus='NAO'
	data-event-keyup='NAO'
...
mostra_botao('insercao','presencas','0');
...
for (i=0; i<pontos.length; i++){
	...
	carrega_NtoN(pontos[i].id,'escolax', 'presencas','id_documento',document.getElementById(pontos[i].id).innerText);
...
ativa_alterados();
if ('presencas'=='documentos'){percorre_ponto_insercao();}
...
mensagem_de_carregamento_ativada();
  ...
  alert('Busca pelo campo '+campo_busca);
	var resposta='';
	var url='insere_presencas.php?offset='+offset+'&limit='+limit+'&campo_busca='+campo_busca+'&valor_busca='+valor_busca+'&scrollx='+escrolx+'&scrolly='+escroly;
        var oReq=new XMLHttpRequest();
	oReq.open('GET',url, false);
	oReq.onload= function (e) {
			    resposta=oReq.responseText;
...
function (){
	var resposta='';
	var url='insere_presencas.php?offset=0&limit=".$limitador_registros_insere."&campo_busca=".$campo_busca."&valor_busca=".$valor_busca."&scrollx='+escrolx+'&scrolly='+escroly;
        var oReq=new XMLHttpRequest();
	oReq.open('GET',url, false);
	oReq.onload= function (e) {
			    resposta=oReq.responseText;
...
var resposta='';
var url='apaga_registro.php?banco=escolax&tabela=presencas&id='+id;
alert(url);
var oReq=new XMLHttpRequest();
oReq.open('GET', url, false);
oReq.onload= function (e) {
