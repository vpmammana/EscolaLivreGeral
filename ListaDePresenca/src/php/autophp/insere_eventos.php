...
<html>
<head>
<title>Inserção de dados na tabela (maio-2020): eventos ".$campo_busca."</title>
...
<body id='conteudo'>
<div class='cabecalio'>
<h1>Inserção de Dados na tabela (maio/2020): eventos</h1>
...
$sql_comment="SELECT TABLE_COMMENT FROM information_schema.TABLES WHERE TABLE_NAME = 'eventos';";
...
<div style='border: 1px solid black; background-color: green; width: 100%'><table style='width: 100%'><tr><td>Consulta:<input type='text' id='id_campo_de_search' data-nivel='0' placeholder='Coloque a consulta aqui!' /></td><td><input 
				type='button'
				value='id_chave_evento' 
				data-nivel='0' 
				name='busca_principal'
				onclick='var itz=document.getElementById(`id_campo_de_search`).value;  if (itz===``) {alert(`Você deixou o campo de busca vazio!`);} else {carrega_busca(`id_chave_evento`,itz,50,0);;}'
			></td><td> <input 
				type='button'
				value='nome_evento' 
				data-nivel='0' 
				name='busca_principal'
				onclick='var itz=document.getElementById(`id_campo_de_search`).value;  if (itz===``) {alert(`Você deixou o campo de busca vazio!`);} else {carrega_busca(`nome_evento`,itz,50,0);;}'
...
<tr class='cabecalio_table'><th></th><th>id</th><th>nome_evento</th><th>descricao</th></tr>";
...
if ($campo_busca=="") {$sql="select * from eventos order by nome_evento limit ".$limit." offset ".$offset;}
else {$sql="select * from eventos where ".$campo_busca." like '".$valor_busca."%' order by '".$campo_busca."' limit ".$limit;}
echo $sql;
$result=$conn->query("$sql");
...
$id_chave_evento=$row["id_chave_evento"];

$id_registro=$row["id_chave_evento"];
...
echo "<tr class='principal'><td>".$row_number."</td><td>".$id_chave_evento."</td><td>
				<input 
...
mostra_botao('insercao','eventos','0');
...
for (i=0; i<pontos.length; i++){
	...
	carrega_NtoN(pontos[i].id,'escolax', 'eventos','id_documento',document.getElementById(pontos[i].id).innerText);
...
ativa_alterados();
if ('eventos'=='documentos'){percorre_ponto_insercao();}
...
mensagem_de_carregamento_ativada();
  ...
  alert('Busca pelo campo '+campo_busca);
	var resposta='';
	var url='insere_eventos.php?offset='+offset+'&limit='+limit+'&campo_busca='+campo_busca+'&valor_busca='+valor_busca+'&scrollx='+escrolx+'&scrolly='+escroly;
        var oReq=new XMLHttpRequest();
	oReq.open('GET',url, false);
	oReq.onload= function (e) {
			    resposta=oReq.responseText;
...
function (){
	var resposta='';
	var url='insere_eventos.php?offset=0&limit=".$limitador_registros_insere."&campo_busca=".$campo_busca."&valor_busca=".$valor_busca."&scrollx='+escrolx+'&scrolly='+escroly;
        var oReq=new XMLHttpRequest();
	oReq.open('GET',url, false);
	oReq.onload= function (e) {
			    resposta=oReq.responseText;
...
var resposta='';
var url='apaga_registro.php?banco=escolax&tabela=eventos&id='+id;
alert(url);
var oReq=new XMLHttpRequest();
oReq.open('GET', url, false);
oReq.onload= function (e) {
